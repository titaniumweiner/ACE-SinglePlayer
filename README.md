...
Open **Server Mods** in the launcher to browse the curated mod library. Installed and ready-to-install entries are listed before unavailable ports. Every entry includes a plain-language description, compatibility result, requirements, source-code links, and a saved-game safety warning. Installable OpenDereth Preview packages include **Three Imbues**, **All-Tier Salvage & Loot Luck**, **Unlimited Stat Augmentation Gems**, **Unlimited Skill Specializations**, and the runtime-only creature and summon options. `CriticalOverride`, `HelloCommand`, `SocietyTailoring`, and **Expanded Cast on Strike** are rebuilt Aquafir or community ports; Expanded Cast on Strike enables the non-Aetheria equipped-item procs documented by [ACEUniqueWeenies](https://github.com/titaniumweiner/ACEUniqueWeenies), while its custom item SQL remains a separate Custom Weenies import. OptimShi's `CustomClothingBase` v1.11 is also an installable Preview using the author's checksum-pinned, unmodified official DLLs after a successful current-ACE loader test. Preview mods are clearly marked as not thoroughly tested in game. Other samples remain listed but unavailable until they are ported and tested. **Import a Mod ZIP...** atomically installs current single-file packages after verifying the embedded SHA-256 hash of every payload file; older sidecar-based packages remain compatible.
